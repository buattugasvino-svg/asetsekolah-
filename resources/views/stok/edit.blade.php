...
<head>
    ...
    <title>Edit Stok - Inventaris</title>
    <style>
        * {
    ...
    <div class="card-body">
        ...
        <form action="{{ route('stok.update', $stok->id_stok) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" name="id_stok" id="id_stok" class="form-control" value="{{ $stok->id_stok }}" readonly>
            </div>

            {{-- 2. ID Barang (Tepat di bawah ID Stok) --}}
            <div class="form-group">
                <label for="id_barang">ID Barang / Pilih Barang <span class="required">*</span></label>
                <select name="id_barang" id="id_barang" class="form-control" required>
                    <option value="">-- Pilih ID Barang --</option>
                    @foreach ($barang as $item)
                        <option value="{{ $item->id_barang }}" {{ old('id_barang', $stok->id_barang) == $item->id_barang ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <input type="number" name="jumlah" id="jumlah" class="form-control" value="{{ old('jumlah', $stok->jumlah) }}" placeholder="Contoh: 50" min="0" required>
            </div>
